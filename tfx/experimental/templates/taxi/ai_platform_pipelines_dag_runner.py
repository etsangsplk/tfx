...
"""Define AiPlatformPipelinesDagRunner to run the pipeline in CAIP pipelines."""
...
from tfx.orchestration.ai_platform_pipelines import ai_platform_pipelines_dag_runner as caipp_runner
...
# - Metadata will be written to managed metadata backend.
...
#       CAIP pipeline), you can use a path starting "gs://YOUR_BUCKET_NAME/path"
#       for DATA_PATH. For example,
#       DATA_PATH = 'gs://bucket/chicago_taxi_trips/csv/'.
DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data')

# TODO(b/156746891): Switch to the default endpoint after CAIP pipeline public
# launch.
CAIPP_EAP_ENDPOINT = 'test-ml.sandbox.googleapis.com'


def run():
  """Define a pipeline to be executed using CAIP pipelines."""
  ...
  runner_config = caipp_runner.AIPlatformPipelinesDagRunnerConfig(
      project_id=project_id,
      display_name='tfx-caip-pipeline-{}'.format(configs.PIPELINE_NAME),
      default_image=tfx_image)

  caipp_runner.AIPlatformPipelinesDagRunner(config=runner_config).run(
